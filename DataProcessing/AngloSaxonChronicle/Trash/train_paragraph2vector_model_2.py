...
for i, entry in enumerate(entries_without_line_break):
    ...
    entry_text = sp(entry)
    tokens = []
    for token in entry_text:
        sentences.append(sentence.text.strip())
    pdb.set_trace()
    chunk = [element for element in chunks(sentences,3)]
...
for i, entry in enumerate(entries_without_line_break):
    ...
    chunk = [element for element in chunks(bow,10)]
    ...
    for f,element in enumerate(chunk):
        tag = str(i)+'_'+str(f)
        chunk_bow = [ ]
        [chunk_bow.append(doc) for doc in element]
        if len(chunk_bow) >9:
            index.append(chunk_bow)
            label_sentence = TaggedDocument(words=chunk_bow,tags=[tag])
            bows.append(label_sentence)

pdb.set_trace()
model_doc2vec = Doc2Vec(bows,min_count=2,window=5,epochs=100)
