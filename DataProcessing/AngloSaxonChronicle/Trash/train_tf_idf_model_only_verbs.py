...
for i, entry in enumerate(entries_without_line_break):
    ...
    for sentence in entry_text.sents:
        ...
        bow.extend(sentence_element)
    bows.append(bow)


# Initialize a gensim dictionary with bows
gensim_dic = gensim_utils.initialize_gensim_dictionary(bows)

# Filter extremes, i.e. everything term the document frequency is below 5
gensim_dic.filter_extremes(no_below=3)

feature_list=gensim_utils.get_df_in_dictionary(gensim_dic,True)
feature_list.to_csv(output_directory +'features_only_verbs.csv')
...
for element in enumerate(matrix_documents):
    if ((np.nonzero(element[1])[0].shape[0] < 3) or
...
with open(output_directory + 'selected_features.json', 'w') as file:
    file.write(json.dumps(features_list))

with open(output_directory + 'removed_entries.json', 'w') as file:
    file.write(json.dumps(entries_for_remove))

np.savetxt(output_directory + 'document_term_matrix.txt', matrix_documents)
...
with open(output_directory + 'index.json', 'w') as file:
